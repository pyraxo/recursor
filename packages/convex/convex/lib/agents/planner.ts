...
export async function executePlanner(
  ...
  // 1. Load context
  const [stack, todos, projectIdea, agentState, reviewerState] = await Promise.all([
    ...
    ctx.runQuery(internal.agentExecution.getAgentState, { stackId, agentType: 'planner' }),
  ...
  const hasWork = checkPlannerHasWork(todos, projectIdea, agentState, reviewerState);
  if (!hasWork.hasWork) {
  ...
  if (reviewerState?.memory?.recommendations?.length > 0) {
    const recommendations = reviewerState.memory.recommendations.join('\n');
    messages.push({
      role: 'user',
  ...
  messages.push({
    role: 'user',
    content: hasWork.prompt || 'What should the team work on next? Create specific todos.',
  });
  ...
  await ctx.runMutation(internal.traces.internalLog, {
    ...
    action: 'planning',
    result: {
      todosCreated: parsed.actions.filter(a => a.type === 'create_todo').length,
...
function checkPlannerHasWork(
  ...
  if (activeTodos.length === 0) {
    return {
      ...
      prompt: 'All current todos are completed. Plan the next phase of development.',
    };
  }
  ...
  if (reviewerState?.memory?.recommendations?.length > 0) {
    return {
      ...
      prompt: 'The reviewer has provided recommendations. Review them and update the plan accordingly.',
    };
  }
  ...
  if (timeSinceLastPlan > 120000) { // 2 minutes
    return {
      hasWork: true,
